...
public final class ClientShowdownState {
    ...
    private static volatile int redScore, blueScore, pendingDrafts;
    ...
    public static void update(SecretOperationsMatchStatePacket packet) {
        ...
        remainingTicks = packet.remainingTicks(); pendingDrafts = packet.pendingDrafts(); waiting = packet.waiting();
        dropProtected = packet.dropProtected(); receivedAtMillis = System.currentTimeMillis();
    }
    public static void clear() { update(new SecretOperationsMatchStatePacket(false, SecretShowdownPhase.IDLE, 0, 0, 0, 0, false, false)); }
