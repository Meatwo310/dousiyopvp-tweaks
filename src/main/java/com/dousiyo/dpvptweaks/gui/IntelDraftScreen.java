...
public class IntelDraftScreen extends Screen {
    ...
    protected void init() {
        ...
        this.cardY = Math.max(58, (this.height - CARD_H) / 2);
        this.rerollX = this.width - REROLL_W - 18;
        this.rerollY = this.height - REROLL_H - 16;
    }
    ...
    public void render(GuiGraphics gg, int mouseX, int mouseY, float partialTick) {
        renderBackdrop(gg);
        gg.drawCenteredString(this.font, "技術選択", this.width / 2, 28, COLOR_TITLE);

        for (int i = 0; i < CARD_COUNT; i++) {
            int x = cardStartX + i * (cardW + cardGap);
            IntelDraftDefinition.ChoiceDefinition choice = i < definition.choices().size() ? definition.choices().get(i) : null;
            renderCard(gg, x, cardY, choice, isInside(mouseX, mouseY, x, cardY, cardW, CARD_H));
        }
    ...
    private void renderCard(GuiGraphics gg, int x, int y, IntelDraftDefinition.ChoiceDefinition choice, boolean hovered) {
        ...
        drawCenteredTrimmed(gg, choice.gun().name(), x + 10, y + 180, cardW - 20, COLOR_TECH);
    }
    ...
    private void renderRerollButton(GuiGraphics gg, int mouseX, int mouseY) {
        boolean enabled = remainingRerolls > 0 && !definition.choices().isEmpty();
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (remainingRerolls > 0 && !definition.choices().isEmpty() && isInside(mouseX, mouseY, rerollX, rerollY, REROLL_W, REROLL_H)) {
            ClientNetwork.CHANNEL.sendToServer(new RerollIntelDraftPacket(definition.sessionId()));
            playClick();
            return true;
        }
    ...
    private void confirm(int choiceIndex) {
        playClick();
        ClientNetwork.CHANNEL.sendToServer(new SelectIntelDraftPacket(definition.sessionId(), choiceIndex));
        this.onClose();
    }
