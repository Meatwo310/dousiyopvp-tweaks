...
public class DpvpTweaksConfigCommand {
    static void register(LiteralArgumentBuilder<CommandSourceStack> builder, RegisterCommandsEvent event) {
        builder.then(Commands.literal("config")
                ...
                .then(Commands.literal("irons")
                        .then(config("overrideAttributes", ServerConfig.IRONS_OVERRIDE_ATTRIBUTES))
                        .then(config("overrideManaRegen", ServerConfig.IRONS_OVERRIDE_MANA_REGEN))
                        .then(config("manaRegenValue", ServerConfig.IRONS_MANA_REGEN_VALUE))
                        .then(config("maxMana", ServerConfig.IRONS_MAX_MANA))
                )
        );
    }
